...
Demo_Status current_status = SoccerDemo;
Demo_Status desired_status = SoccerDemo;
...
int main(int argc, char **argv)
{
  ...
  // default_mp3_path = ament_index_cpp::get_package_share_directory("op3_soccer") + "/data/mp3/";
  ...
  while (rclcpp::ok())
  {
    // Always force SoccerDemo mode
    desired_status = SoccerDemo;
    current_status = SoccerDemo;
    apply_desired = false;

    if (soccer_demo->isDemoEnabled() == true)
    {
      soccer_demo->process();
    }
